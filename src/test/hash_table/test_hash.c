...
void test_hash_table_apply_or_insert()
{
  ...
  void test(short kmer_size, int num_bits, int bucket, int max_tries, int step, long long max_kmer_to_test)
      ...
      for (i=0; i< max_kmer_to_test; i=i+step)
	{
	  ...
	  applied = hash_table_apply_or_insert(element_get_key(&b,hash_table->kmer_size, &tmp_kmer),&db_node_action_set_flag_pruned , hash_table);
	  CU_ASSERT(applied==true);
	  dBNode * node = hash_table_find(element_get_key(&b,hash_table->kmer_size, &tmp_kmer),hash_table);
	  CU_ASSERT(flags_check_for_flag(PRUNED,&(node->flags))==true);
	}
