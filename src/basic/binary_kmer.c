...
static void shift_left(BinaryKmer * kmer, int num_bits_to_shift, short kmer_size, int which_bitfield_in_kmer,	bitfield_of_64bits overflow_to_apply_from_bitfield_to_the_right) {
    ...
    if (which_bitfield_in_kmer >= NUMBER_OF_BITFIELDS_IN_BINARY_KMER - 1 - number_of_bitfields_fully_used) {
        shift_left(kmer,num_bits_to_shift,kmer_size, which_bitfield_in_kmer, new_overflow);
    }
...
void binary_kmer_left_shift(BinaryKmer * kmer, int num_bits_to_shift, short kmer_size)
{
	...
	shift_left(kmer,num_bits_to_shift,kmer_size, NUMBER_OF_BITFIELDS_IN_BINARY_KMER - 1, 0);
...
static void shift_right(BinaryKmer * kmer, int num_bits_to_shift, int which_bitfield_in_kmer,
                        bitfield_of_64bits overflow_to_apply_from_bitfield_to_the_left){
...
void binary_kmer_to_stream(BinaryKmer * bkmer, short kmer_size, FILE * f){
    char * tmp_str = calloc(kmer_size + 1, sizeof(char));
    
    binary_kmer_to_seq(bkmer,kmer_size, tmp_str);
    fprintf(f, "[kmer]%p:%s\n",&bkmer, tmp_str );
}
